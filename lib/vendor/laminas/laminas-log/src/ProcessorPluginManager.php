...
class ProcessorPluginManager extends AbstractPluginManager
{
    protected $aliases = [
        ...
        \_JchOptimizeVendor\Zend\Log\Processor\Backtrace::class => Processor\Backtrace::class,
        \_JchOptimizeVendor\Zend\Log\Processor\PsrPlaceholder::class => Processor\PsrPlaceholder::class,
        \_JchOptimizeVendor\Zend\Log\Processor\ReferenceId::class => Processor\ReferenceId::class,
        \_JchOptimizeVendor\Zend\Log\Processor\RequestId::class => Processor\RequestId::class,
        // v2 normalized FQCNs
    ...
    public function validatePlugin($plugin)
    {
        try {
            ...
            throw new Exception\InvalidArgumentException(\sprintf('_JchOptimizeVendor\\Plugin of type %s is invalid; must implement %s\\Processor\\ProcessorInterface', \is_object($plugin) ? \get_class($plugin) : \gettype($plugin), __NAMESPACE__));
        }
    }
}
