...
use _JchOptimizeVendor\Laminas\Log\Filter;
use _JchOptimizeVendor\Laminas\Log\FilterPluginManager as LogFilterPluginManager;
use _JchOptimizeVendor\Laminas\Log\Formatter;
use _JchOptimizeVendor\Laminas\Log\FormatterPluginManager as LogFormatterPluginManager;
...
abstract class AbstractWriter implements WriterInterface
{
     ...
     * @var bool
     */
    protected $convertWriteErrorsToExceptions = \true;
    /**
    ...
    public function setFilterPluginManager($plugins)
    {
        ...
        if (!$plugins instanceof LogFilterPluginManager) {
            throw new Exception\InvalidArgumentException(\sprintf('Writer plugin manager must extend %s; received %s', LogFilterPluginManager::class, \is_object($plugins) ? \get_class($plugins) : \gettype($plugins)));
        }
    ...
    public function setFormatterPluginManager($plugins)
    {
        ...
        if (!$plugins instanceof LogFormatterPluginManager) {
            throw new Exception\InvalidArgumentException(\sprintf('Writer plugin manager must extend %s; received %s', LogFormatterPluginManager::class, \is_object($plugins) ? \get_class($plugins) : \gettype($plugins)));
        }
