...
class LoggerAbstractServiceFactory implements AbstractFactoryInterface
{
    ...
    protected function processConfig(&$config, ContainerInterface $services)
    {
        ...
        foreach ($config['writers'] as $index => $writerConfig) {
            if (isset($writerConfig['name']) && ('db' === $writerConfig['name'] || Writer\Db::class === $writerConfig['name'] || 'laminaslogwriterdb' === $writerConfig['name']) && isset($writerConfig['options']['db']) && \is_string($writerConfig['options']['db']) && $services->has($writerConfig['options']['db'])) {
            ...
            if (isset($writerConfig['name']) && ('mongo' === $writerConfig['name'] || Writer\Mongo::class === $writerConfig['name'] || 'laminaslogwritermongo' === $writerConfig['name']) && isset($writerConfig['options']['mongo']) && \is_string($writerConfig['options']['mongo']) && $services->has($writerConfig['options']['mongo'])) {
            ...
            if (isset($writerConfig['name']) && ('mongodb' === $writerConfig['name'] || Writer\MongoDB::class === $writerConfig['name'] || 'laminaslogwritermongodb' === $writerConfig['name']) && isset($writerConfig['options']['manager']) && \is_string($writerConfig['options']['manager']) && $services->has($writerConfig['options']['manager'])) {
