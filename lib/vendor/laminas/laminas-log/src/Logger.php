...
use DateTime;
use ErrorException;
use _JchOptimizeVendor\Laminas\Log\Processor\ProcessorInterface;
...
use _JchOptimizeVendor\Laminas\Stdlib\SplPriorityQueue;
use Traversable;
/**
...
class Logger implements LoggerInterface
{
     ...
     * @var array
     */
    protected $priorities = [self::EMERG => 'EMERG', self::ALERT => 'ALERT', self::CRIT => 'CRIT', self::ERR => 'ERR', self::WARN => 'WARN', self::NOTICE => 'NOTICE', self::INFO => 'INFO', self::DEBUG => 'DEBUG'];
    /**
     * Writers
     *
    ...
    public function __construct($options = null)
    {
        ...
        if (isset($options['exceptionhandler']) && $options['exceptionhandler'] === \true) {
            static::registerExceptionHandler($this);
        }
        if (isset($options['errorhandler']) && $options['errorhandler'] === \true) {
            static::registerErrorHandler($this);
        }
        if (isset($options['fatal_error_shutdownfunction']) && $options['fatal_error_shutdownfunction'] === \true) {
    ...
    public function setWriters(SplPriorityQueue $writers)
    {
        foreach ($writers->toArray() as $writer) {
            if (!$writer instanceof Writer\WriterInterface) {
                throw new Exception\InvalidArgumentException('_JchOptimizeVendor\\Writers must be a SplPriorityQueue of Laminas\\Log\\Writer');
            }
        }
    ...
    public function addProcessor($processor, $priority = 1, array $options = null)
    {
        if (\is_string($processor)) {
            ...
            throw new Exception\InvalidArgumentException(\sprintf('Processor must implement Laminas\\Log\\ProcessorInterface; received "%s"', \is_object($processor) ? \get_class($processor) : \gettype($processor)));
        }
    ...
    public function log($priority, $message, $extra = [])
    {
        ...
        $event = ['timestamp' => $timestamp, 'priority' => (int) $priority, 'priorityName' => $this->priorities[$priority], 'message' => (string) $message, 'extra' => $extra];
    ...
    public static function registerErrorHandler(Logger $logger, $continueNativeHandler = \false)
    {
        ...
        $previous = \set_error_handler(function ($level, $message, $file, $line) use($logger, $errorPriorityMap, $continueNativeHandler) {
            ...
            if ($iniLevel & $level) {
                ...
                $logger->log($priority, $message, ['errno' => $level, 'file' => $file, 'line' => $line]);
            }
            return !$continueNativeHandler;
        });
        static::$registeredErrorHandler = \true;
        return $previous;
    }
    /**
    ...
    public static function registerFatalErrorShutdownFunction(Logger $logger)
    {
        ...
        \register_shutdown_function(function () use($logger, $errorPriorityMap) {
            ...
            if (null === $error || !\in_array($error['type'], [\E_ERROR, \E_PARSE, \E_CORE_ERROR, \E_CORE_WARNING, \E_COMPILE_ERROR, \E_COMPILE_WARNING], \true)) {
                return;
            }
            $logger->log($errorPriorityMap[$error['type']], $error['message'], ['file' => $error['file'], 'line' => $error['line']]);
        });
        static::$registeredFatalErrorShutdownFunction = \true;
        return \true;
    }
    /**
    ...
    public static function registerExceptionHandler(Logger $logger)
    {
        ...
        \set_exception_handler(function ($exception) use($logger, $errorPriorityMap) {
            ...
            do {
                ...
                $extra = ['file' => $exception->getFile(), 'line' => $exception->getLine(), 'trace' => $exception->getTrace()];
                if (isset($exception->xdebug_message)) {
                    $extra['xdebug'] = $exception->xdebug_message;
                }
                $logMessages[] = ['priority' => $priority, 'message' => $exception->getMessage(), 'extra' => $extra];
        ...
        static::$registeredExceptionHandler = \true;
        return \true;
    }
    /**
