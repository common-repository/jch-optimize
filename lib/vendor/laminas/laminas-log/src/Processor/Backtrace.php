...
namespace _JchOptimizeVendor\Laminas\Log\Processor;
...
class Backtrace implements ProcessorInterface
{
     ...
     * @var array
     */
    protected $ignoredNamespaces = ['_JchOptimizeVendor\\Laminas\\Log'];
    /**
    ...
    public function process(array $event)
    {
        ...
        \array_shift($trace);
        // ignore $this->getBacktrace();
        \array_shift($trace);
        // ignore $this->process()
        $i = 0;
        while (isset($trace[$i]['class']) && $this->shouldIgnoreFrame($trace[$i]['class'])) {
            $i++;
        }
        $origin = ['file' => isset($trace[$i - 1]['file']) ? $trace[$i - 1]['file'] : null, 'line' => isset($trace[$i - 1]['line']) ? $trace[$i - 1]['line'] : null, 'class' => isset($trace[$i]['class']) ? $trace[$i]['class'] : null, 'function' => isset($trace[$i]['function']) ? $trace[$i]['function'] : null];
        $extra = $origin;
