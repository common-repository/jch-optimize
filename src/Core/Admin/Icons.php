...
HTML;
    ...
    public function compileAutoSettingsIcons($settings): array
    {
        ...
        for ($i = 0; $i < count($settings); $i++) {
            ...
            $buttons[$i]['script'] = "onclick=\"jchPlatform.applyAutoSettings('{$settings[$i]['setting']}', '{$id}', '" . Utility::getNonce(
    ...
    public function getUtilityArray(array $actions = []): array
    {
        $aUtilities = [
            ($action = 'browsercaching')  => [
                ...
                'tooltip' => Utility::translate(
                    'Use this button to add codes to your htaccess file to enable leverage browser caching and gzip compression.'
                )
            ],
            ($action = 'filepermissions') => [
                ...
                'tooltip' => Utility::translate(
                    "If your site has lost CSS formatting after enabling the plugin, the problem could be that the plugin files were installed with incorrect file permissions so the browser cannot access the cached combined file. Click here to correct the plugin's file permissions."
                )
            ],
            ($action = 'cleancache')      => [
                'action'  => $action,
                ...
                'name'    => 'Clean Cache',
                'tooltip' => Utility::translate(
                    "Click this button to clean the plugin's cache and page cache. If you have edited any CSS or JavaScript files you need to clean the cache so the changes can be visible."
                )
            ],
            ($action = 'orderplugins')    => [
                'action'  => $action,
                ...
                'name'    => 'Order Plugin',
                'tooltip' => Utility::translate(
                    'The published order of the plugin is important! When you click on this icon, it will attempt to order the plugin correctly.'
                )
            ],
            ($action = 'keycache')        => [
                'action'  => $action,
                ...
                'tooltip' => Utility::translate(
                    "If you've made any changes to your files generate a new cache key to counter browser caching of the old content."
                )
            ],
            ($action = 'recache')         => [
                'action'  => $action,
            ...
            ($action = 'bulksettings')    => [
                'action'  => $action,
                'icon'    => 'bulk_settings.png',
                'name'    => 'Bulk Setting Operations',
                'tooltip' => Utility::translate(
            ...
            ($action = 'restoreimages')   => [
                ...
                'tooltip' => Utility::translate(
                    "If you're not satisfied with the images that were optimized you can restore the original ones by clicking this button if they were not deleted. This will also remove any webp image created from the restored file."
                ),
                'proonly' => true,
            ],
            ($action = 'deletebackups')   => [
                ...
                'tooltip' => Utility::translate(
                    "This will permanently delete the images that were backed up. There's no way to undo this so be sure you're satisfied with the ones that were optimized before clicking this button."
                ),
                'proonly' => true,
    ...
    public function getToggleSettings(): array
    {
        ...
        return [
            ...
            [
                ...
                'enabled' => $this->params->get($setting, '0'),
                'proonly' => true,
                'tooltip' => Utility::translate('Optimizes the loading of fonts, including Google Fonts.')
            ],
            [
                'name'    => 'CDN',
                ...
                'enabled' => $this->params->get($setting, '0'),
                'tooltip' => Utility::translate(
                    'Loads static assets from a CDN server. Requires the CDN domain(s) to be configured on the Configuration tab.'
                )
            ],
            [
                'name'    => 'Smart Combine',
                ...
                'proonly' => true,
                'tooltip' => Utility::translate(
                    'Intelligently combines files in a number of smaller files, instead of one large file for better http2 delivery.'
                )
            ],
            [
                'name'    => 'Load Webp',
                ...
                'proonly' => true,
                'tooltip' => Utility::translate(
                    'Loads generated WEBP images in place of the original ones. These images must be generated on the Optimize Image tab first.'
                )
            ],
            [
                'name'    => 'Page Cache',
                ...
                'icon'    => 'cache.png',
                'enabled' => Cache::isPageCacheEnabled($this->params),
    ...
    public function getAdvancedToggleSettings(): array
    {
        return [
            [
                ...
                'icon'    => 'reduce_unused_css.png',
                'enabled' => $this->params->get($setting, '0'),
                'proonly' => true,
                'tooltip' => Utility::translate(
                    'Loads only the critical CSS required for rendering the page above the fold until user interacts with the page. Requires Optimize CSS Delivery to be enabled and may need the CSS Dynamic Selectors setting to be configured to work properly.'
                )
            ],
            [
                'name'    => 'Reduce Unused JavaScript',
                'setting' => ($setting = 'pro_reduce_unused_js_enable'),
                'icon'    => 'reduce_unused_js.png',
                'enabled' => $this->params->get($setting, '0'),
                'proonly' => true,
                'tooltip' => Utility::translate(
                    'Will defer the loading of JavaScript until the user interacts with the page to improve performance affected by unused JavaScript. If your site uses JavaScript to perform the initial render you may need to \'exclude\' these critical JavaScript. These will be bundled together, preloaded and loaded asynchronously.'
                )
            ],
            [
                'name'    => 'Reduce DOM',
